var jsPsychSurveyTextTimeout = (function (jspsych) {
  ...
  class SurveyTextPlugin {
      ...
      trial(display_element, trial) {
          ...
          if (trial.randomize_question_order) {
              question_order = this.jsPsych.randomization.shuffle(question_order);
                   ...
                   // Function to run after the Enter key is pressed after the timeout
    ...
    document.addEventListener('keydown', (event) => {
            ...
            if (event.key === 'Enter') {
                if (isDisabled) {
                    // If the Enter key is disabled, prevent default action
                    event.preventDefault();
                    console.log("Enter key deactivated")
                } 
                else {
                    // If the Enter key is active, call the function
                    this.jsPsych.pluginAPI.clearAllTimeouts(function (){
                        display_element.querySelector("#jspsych-survey-text-form").requestSubmit();
                    });
               isDisabled=true
                setTimeout(() => {
                    isDisabled = false; console.log(isDisabled, "Enter key is enabled") // Re-enable the Enter key
                }, 505);
                console.log(isDisabled)
                }
            }
